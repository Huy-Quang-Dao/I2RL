...
class DDP():
    ...
    def backward(self, x_seq, u_seq):
        ...
        for t in range(self.pred_time - 1, -1, -1):
            ...
            self.p[t] = Q_x - np.matmul(np.matmul(Q_u, inv_Q_uu), Q_ux)
            self.P[t] = Q_xx + np.matmul(Q_ux.T, K)
            d_k.append(d)
            K_k.append(K)
        d_k.reverse()
        K_k.reverse()
...
if __name__ == '__main__':
    ...
    np.save('DDP\policies3.npy', policies)
    # policies = np.load('DDP\policies3.npy')
    # print(policies)
