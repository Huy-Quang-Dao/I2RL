...
class DDP():
    ...
    def compute_local_expansions(self,s, a):
        f_x = lambda state,action: self.jacobian(lambda x: self.f(x, action), state)
        f_u = lambda state,action: self.jacobian(lambda x: self.f(state, x), action)
        return (
            ...
            self.jacobian(lambda x: self.vec(f_x(x,a)),s),
            self.jacobian(lambda x: self.vec(f_u(s,x)),a),
            self.jacobian(lambda x: self.vec(f_x(s,x)),a),
        )
    ...
    def backward(self, x_seq, u_seq):
        ...
        for t in range(self.pred_time - 1, -1, -1):
            ...
            l_x = compute_at_k[5]
            l_u = compute_at_k[6]
            dAkdx = compute_at_k[7]
            dBkdu = compute_at_k[8]
            dAkdu = compute_at_k[9]
            n,m = B_k.shape

            Q_x = l_x + np.matmul(A_k.T, self.p[t + 1])  # \nabla_x(l) + A_t^T @ p_{k+1}
            Q_u = l_u + np.matmul(B_k.T, self.p[t + 1])  # \nabla_u(l) + B_t^T @ p_{k+1}
            T_comA = self.commutation_matrix_sp(A_k) # commutator matrix
            T_comB = self.commutation_matrix_sp(B_k) # commutator matrix

            Q_xx = (
                l_xx
                + np.matmul(np.matmul(A_k.T, self.P[t + 1]), A_k)
                + (np.kron(self.p[t+1], np.eye(n))) @ T_comA @ dAkdx
            )  # \nabla_xx(l) + A_k^T @ P_{t+1} @ A_k + (kron(p_{k+1},I)) @ T @ dAkdx
            Q_uu = (
                l_uu
                + np.matmul(np.matmul(B_k.T, self.P[t + 1]), B_k)
                + (np.kron(self.p[t+1], np.eye(m))) @ T_comB @ dBkdu
            ) # \nabla_uu(l) + B_k^T @ P_{t+1} @ B_k + (kron(p_{k+1},I)) @ T @ dBkdu

            Q_xu = (
                l_xu
                + np.matmul(np.matmul(A_k.T, self.P[t + 1]), B_k)
                + (np.kron(self.p[t+1], np.eye(n))) @ T_comA @ dAkdu
            )  # \nabla_xu(l) + A_k^T @ P_{t+1} @ B_k + (kron(p_{k+1},I)) @ T @ dAkdu


            inv_Q_uu = np.linalg.inv(Q_uu)
            d = np.matmul(inv_Q_uu, Q_u)
            K = np.matmul(inv_Q_uu, Q_xu.T)
            self.p[t] = Q_x - np.matmul(K.T,Q_u) + np.matmul(np.matmul(K.T,Q_uu),d) - np.matmul(Q_xu,d)
            self.P[t] = Q_xx + np.matmul(np.matmul(K.T,Q_uu),K) - np.matmul(Q_xu,K) - np.matmul(K.T,Q_xu.T)
            d_k.append(d)
            K_k.append(K)
        d_k.reverse()
        K_k.reverse()
...
if __name__ == '__main__':
    ...
    # np.save('DDP\policies.npy', policies)
    policies = np.load('DDP\policies.npy')
    # print(policies)
